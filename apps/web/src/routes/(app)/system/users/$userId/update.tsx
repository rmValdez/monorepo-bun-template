...
import { type TUpdateUserSchemaInput, updateUserSchema } from "@/server/users/users.schema";

export const Route = createFileRoute("/(app)/system/users/$userId/update")({
...
function RouteComponent() {
  ...
  const defaultRoleIds = user.members?.flatMap((m) => m.memberRoles?.map((mr) => mr.roleId)) || [];
  const defaultOrgId = user.members?.[0]?.organizationId || "";
    ...
    <div className="w-full max-w-4xl mx-auto py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Modify user administration profile.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the fields below to modify user credentials or permissions.</CardDescription>
        </CardHeader>
        <CardContent>
          <form
            onSubmit={(e) => {
