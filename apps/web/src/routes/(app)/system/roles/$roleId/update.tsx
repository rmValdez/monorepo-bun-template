...
import { updateRoleWithPermissionsFn } from "@/server/roles-permission/role-permission.functions";
import { rolePermissionQueries } from "@/server/roles-permission/role-permission.queries";
import {
  type TUpdateRoleWithPermissionsSchemaInput,
  updateRoleWithPermissionsSchema,
} from "@/server/roles-permission/role-permission.schema";

export const Route = createFileRoute("/(app)/system/roles/$roleId/update")({
...
function RouteComponent() {
  ...
  const defaultRoleValues: TUpdateRoleWithPermissionsSchemaInput = {
    id: role.id,
    name: role.name,
  ...
  const roleForm = useAppForm({
    ...
    validators: {
      onChange: updateRoleWithPermissionsSchema,
      onSubmit: updateRoleWithPermissionsSchema,
    },
    onSubmit: async ({ value }) => {
      updateRoleWithPermissionsMutation.mutate({ data: value });
    },
  });

  const updateRoleWithPermissionsMutation = useMutation({
    mutationFn: updateRoleWithPermissionsFn,
    onSuccess: (response) => {
      if (!response.success) {
    ...
    <div className="w-full max-w-4xl mx-auto py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Modify role details and permissions.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the role and its permissions below.</CardDescription>
        </CardHeader>
        <CardContent>
          <form
            onSubmit={(e) => {
          ...
          >
            <FieldSet>
              ...
              <FieldDescription>Basic information for the role.</FieldDescription>
              <FieldGroup>
