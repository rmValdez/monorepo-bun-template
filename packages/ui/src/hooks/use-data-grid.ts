...
import { useDirection } from "@workspace/ui/components/ui/direction";
...
import {
  ...
  parseTsv,
  scrollCellIntoView,
} from "@workspace/ui/lib/data-grid";
import type {
  CellPosition,
  CellUpdate,
  ContextMenuState,
  ...
  SearchState,
  SelectionState,
} from "@workspace/ui/types/data-grid";
...
function useDataGrid<TData>({
  ...
  const onDataUpdate = React.useCallback(
    (updates: CellUpdate | Array<CellUpdate>) => {
      ...
      const maxUpdateIndex = rowUpdatesMap.size > 0 ? Math.max(...Array.from(rowUpdatesMap.keys())) : -1;
  ...
  const onCellsPaste = React.useCallback(
    async (expandRows = false) => {
      ...
      try {
        ...
        const isSingleCellClipboard = rawPastedData.length === 1 && (rawPastedData[0]?.length ?? 0) === 1;

        let pastedData = rawPastedData;
        let startRowIndex = currentState.focusedCell.rowIndex;
        let startColIndex = navigableColumnIds.indexOf(currentState.focusedCell.columnId);
        ...
        if (isSingleCellClipboard && selectionCells.size > 1) {
          ...
          if (minRow !== Infinity) {
            ...
            pastedData = Array.from({ length: numRows }, () => Array.from({ length: numCols }, () => singleValue));
          }
        }
  ...
  const onCellClick = React.useCallback(
    ...
    [store, focusCell, scrollToCell, onCellEditingStart, selectRange, onSelectionClear],
  );
      ...
      scrollToCell: (rowIndex, columnId, align = "auto") => {
        ...
        const scrollRowIntoView = (retries = 1) => {
          requestAnimationFrame(() => {
            ...
            const headerBottom =
              headerRef.current?.getBoundingClientRect().bottom ?? container.getBoundingClientRect().top;
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    scrollPaddingEnd:
      ...
      (footerRef.current?.getBoundingClientRect().top ?? dataGridRef.current?.getBoundingClientRect().bottom ?? 0) +
      rowHeightValue +
      VIEWPORT_OFFSET,
  });
  ...
  React.useEffect(() => {
    ...
    function getAutoScrollSpeed(dist: number): number {
      ...
      return Math.round(AUTO_SCROLL_MIN_SPEED + (AUTO_SCROLL_MAX_SPEED - AUTO_SCROLL_MIN_SPEED) * t);
    }
    ...
    function tick() {
      ...
      if (dx !== 0) {
        ...
        if (relX < leftZoneWidth) {
          const columns = isActuallyRtl ? tbl.getRightVisibleLeafColumns() : tbl.getLeftVisibleLeafColumns();
          ...
          const columns = isActuallyRtl ? tbl.getLeftVisibleLeafColumns() : tbl.getRightVisibleLeafColumns();
