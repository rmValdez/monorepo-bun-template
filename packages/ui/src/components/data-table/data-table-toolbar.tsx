...
export function DataTableToolbar<TData>({
  ...
  const isSorted = JSON.stringify(table.getState().sorting) !== JSON.stringify(table.options.initialState?.sorting ?? []);
  const isDirty = isFiltered || isSearched || isSorted;
  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-wrap items-center gap-2">
        ...
        {isDirty && (
          <Button aria-label="Reset filters" variant="outline" className="border-dashed" onClick={handleReset}>
            <X />
            Reset
          </Button>
        )}
