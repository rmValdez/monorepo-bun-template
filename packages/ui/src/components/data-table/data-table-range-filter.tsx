  ...
  return (
    <div data-slot="range" className={cn("flex w-full items-center gap-2", className)} {...props}>
      <Input
        ...
        onChange={(event: React.ChangeEvent<HTMLInputElement>) => onRangeValueChange(event.target.value, true)}
      />
      ...
      <Input
        ...
        onChange={(event: React.ChangeEvent<HTMLInputElement>) => onRangeValueChange(event.target.value)}
      />
    </div>
  );
}
