...
export function DataTableSortList<TData>({ table, disabled, className, ...props }: DataTableSortListProps<TData>) {
  ...
  const onSortRemove = React.useCallback(
    (sortId: string) => {
      onSortingChange((prevSorting) => prevSorting.filter((item: ColumnSort) => item.id !== sortId));
    },
    [onSortingChange],
  );
  ...
  const isSortingModified =
    sorting.length !== initialSorting.length ||
    sorting.some((s, i) => s.id !== initialSorting[i]?.id || s.desc !== initialSorting[i]?.desc);

  return (
    <Sortable value={sorting} onValueChange={onSortingChange} getItemValue={(item: ColumnSort) => item.id}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <div className="flex w-full items-center gap-2">
            <Button className="rounded" ref={addButtonRef} onClick={onSortAdd} disabled={columns.length === 0}>
              Add sort
            </Button>
            {isSortingModified && (
    ...
    <SortableItem value={sort.id}>
      ...
      <div role="listitem" id={sortItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown}>
        ...
        <Select
          ...
          onValueChange={(value: string | null) => onSortUpdate(sort.id, { desc: value === "desc" })}
        >
          ...
          <SelectContent id={directionListboxId} className="min-w-(--radix-select-trigger-width)">
            <SelectGroup>
              {dataTableConfig.sortOrders.map((order: { label: string; value: string }) => (
        ...
        <SortableItemHandle className={buttonVariants({ variant: "outline", size: "icon", className: "size-8 shrink-0 rounded" })}>
          <GripVertical />
        </SortableItemHandle>
      </div>
    </SortableItem>
