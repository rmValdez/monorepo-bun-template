/** biome-ignore-all lint/a11y/useSemanticElements: <component> */
"use client";
...
import { DataGridCellWrapper } from "@workspace/ui/components/data-grid/data-grid-cell-wrapper";
...
import { useBadgeOverflow } from "@workspace/ui/hooks/use-badge-overflow";
...
import type { DataGridCellProps, FileCellData } from "@workspace/ui/types/data-grid";
...
export function CheckboxCell<TData>({
  ...
  const onCheckboxClick: NonNullable<React.ComponentProps<typeof Checkbox>["onClick"]> = React.useCallback((event) => {
    event.stopPropagation();
  }, []);

  const onCheckboxMouseDown: NonNullable<React.ComponentProps<typeof Checkbox>["onMouseDown"]> = React.useCallback(
    (event) => {
      event.stopPropagation();
    },
    [],
  ...
  const onCheckboxDoubleClick: NonNullable<React.ComponentProps<typeof Checkbox>["onDoubleClick"]> = React.useCallback(
    (event) => {
      event.stopPropagation();
    },
    [],
...
export function SelectCell<TData>({
  ...
  const displayLabel = value ? (optionByValue.get(value)?.label ?? value) : null;
